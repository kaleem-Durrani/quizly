...
/**
 * Get subjects formatted for a select component (value/label pairs)
 * @route GET /api/subjects/select
 * @access Private
 */
export const getSubjectsForSelect = asyncHandler(async (req: Request, res: Response) => {
  // Only get active subjects by default, unless explicitly requesting all
  ...
  // Get subjects sorted by name
  const subjects = await Subject.find(query)
    .sort({ name: 1 })
    .select('_id name');

  // Format subjects as value/label pairs for select components
  const selectOptions: SelectOption[] = subjects.map(subject => ({
    value: (subject._id as mongoose.Types.ObjectId).toString(),
    label: subject.name,
  }));
