...
import { Typography, Card, Tabs, Button, Table, Space, Tag, Modal, message } from 'antd';
import { ArrowLeftOutlined, ReloadOutlined, PlusOutlined, DeleteOutlined } from '@ant-design/icons';
...
const ClassDetail: React.FC = () => {
  ...
  const handleRegenerateJoinCode = () => {
    confirm({
      ...
      content: 'Are you sure you want to regenerate the join code? The old code will no longer work.',
      onOk: async () => {
    ...
    },
    {
      ...
      render: (record: any) => (
        <Button 
          ...
          danger 
          size="small"
          onClick={() => handleRemoveStudent(record.id, `${record.firstName} ${record.lastName}`)}
        >
          Remove
        </Button>
      ),
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <Space size="small">
          ...
          <Link to={generatePath(ROUTES.TEACHER.QUIZ_RESULTS, { id: record.id })}>
        ...
        <div className="flex justify-between items-start">
          <div>
            <div className="mb-2">
              ...
              <Tag color="blue" className="text-lg">{classData.joinCode}</Tag>
              <Button 
                icon={<ReloadOutlined />} 
                size="small" 
                className="ml-2"
